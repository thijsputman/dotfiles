...
rclone sync ~/esphome/config --max-depth 0 --exclude secrets.yaml \
...
sed -E 's/^(wifi_[a-z0-9_]+:).*/\1/Ig' ~/esphome/config/secrets.yaml |
  rclone rcat onedrive:AppData/WSL/esphome/secrets.yaml
rclone touch onedrive:AppData/WSL/esphome/secrets.yaml -t \
  "$(
